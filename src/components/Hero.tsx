...
export const Hero: React.FC<HeroProps> = ({ event = 'both', inviteeName }) => {
  ...
  return (
    <div ref={containerRef} className="relative h-screen flex items-center justify-center overflow-hidden bg-brand-blush/30">
      ...
      <div className="absolute inset-5 sm:inset-8 border border-brand-plum/30 rounded-3xl pointer-events-none z-20 hidden sm:block shadow-[inset_0_0_30px_rgba(15,23,42,0.1)]" />
      ...
      >
        ...
        >
          ...
          <div className="mb-6 sm:mb-10 inline-block bg-gradient-to-r from-brand-rose/90 via-white/95 to-brand-rose/90 backdrop-blur-md border border-brand-plum/40 px-8 sm:px-10 py-2.5 sm:py-3 rounded-full shadow-[0_10px_30px_rgba(15,23,42,0.2)]">
            <span className="text-stone-900 uppercase tracking-[0.6em] sm:tracking-[0.8em] text-[10px] sm:text-xs font-black block drop-shadow-sm font-sans">
              We’ve decided to keep each other!
            </span>
          </div>
          ...
          <div className="relative mb-6 sm:mb-12 w-full flex justify-center py-4 sm:py-10 px-2 overflow-visible">
            ...
            <h1 className="relative text-4xl sm:text-[5.5rem] lg:text-[7.5rem] font-names text-[#0F172A] font-normal leading-normal sm:leading-[0.85] drop-shadow-[0_4px_16px_rgba(255,255,255,0.9)] tracking-normal sm:tracking-tight overflow-visible py-2">
              MIHIRI <br className="sm:hidden" />
              <span className="text-[#0F172A] font-normal mx-2 sm:mx-6 text-3xl sm:text-[4.5rem] lg:text-[6rem] inline-block -translate-y-1 sm:-translate-y-6 drop-shadow-[0_4px_12px_rgba(15,23,42,0.3)]">&</span>
